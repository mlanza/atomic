...
* There is no reason for a `Just` type (i.e. the Maybe monad).  `Nil`, the surrogate for `None` (or null) gives us this automatically.  Maybe is thus, the sum type of `Nil`  and `Any` (everything else) and we have these without adding a layer of monadic wrapping by virtual of protocols properly applied.  A functor on `fmap` must preserve the original type (wrapper) but recall that sum types unite separate types into a conceptual whole, thus `Nil | Any` is an all-inclusive sum type.
