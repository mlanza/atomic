...
export {log, juxt, isTrue, isFalse, see, rand, overload, constantly, comp, pipe, chain, maybe, opt, partial, isOdd, isEven, memoize, doto, supply, fst, snd, spread, unspread, nullary, unary, binary, inc, dec, slice, identity} from './core';
export {extend} from './protocol';
import {satisfies as _satisfies} from './protocol';
export {sort, dedupe, distinct, doall, repeatedly, expanding, fnil, flatten, reverse, cat, someFn, everyPred, cycle, range, butlast, expansive, groupBy, most, least, amplify, then, toObject, toArray} from './coll';
...
export const insert       = curry(c.insert);
export const interpret    = curry(c.interpret);
...
export const intersection = curry(c.intersection);
export const isSuperset   = curry(c.isSuperset);
export const difference   = curry(c.difference);
export const union        = curry(c.union, 2);
...
export const cons         = curry(flop(c.cons, 2));
export const conj         = curry(flop(_conj, 2));
export const has          = curry(flop(_has, 2));
...
export const gt           = curry(flop(c.gt, 2));
export const gte          = curry(flop(c.gte, 2));
export const lt           = curry(flop(c.lt, 2));
export const lte          = curry(flop(c.lte, 2));
...
export const assoc        = curry(flop(_assoc, 3));
export const dissoc       = curry(flop(_dissoc, 2));
export const hasKey       = curry(flop(_hasKey, 2));
export const keys         = curry(flop(_keys, 2));
export const vals         = curry(flop(_vals, 2));
export const swap         = curry(flop(_swap, 2));
export const reset        = curry(flop(_reset, 2));
export const publish      = curry(flop(_publish, 2));
export const subscribe    = curry(flop(_subscribe, 2));
export const query        = subj(_query);
export const fetch        = curry(flop(_fetch, 2));
export const get          = curry(flop(_get, 2));
export const append       = curry(flop(_append, 2));
export const prepend      = curry(flop(_prepend, 2));
export const reduce       = curry(flop(_reduce, 3));
export const reduceKV     = curry(flop(_reduceKv, 3));
export const closest      = curry(flop(_closest, 2));
export const nth          = curry(flop(_nth, 2));
export const hasClass     = curry(flop(el.hasClass));
export const addClass     = curry(flop(el.addClass));
export const removeClass  = curry(flop(el.removeClass));
...
export const assocIn      = curry(flop(c.assocIn, 3));
export const getIn        = curry(flop(c.getIn, 2));
...
export const attr         = curry(flop(el.attr, 2));
export const css          = curry(flop(el.css, 2));
...
export const startsWith   = curry(flop(string.startsWith, 2));
export const endsWith     = curry(flop(string.endsWith, 2));
export const replace      = curry(flop(string.replace, 3));