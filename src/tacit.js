...
export {log, juxt, see, rand, overload, constantly, comp, pipe, chain, maybe, opt, partial, isOdd, isEven, firstly as coalesce} from './core';
export {extend} from './protocol';
import {satisfies as _satisfies} from './protocol';
export {sort, dedupe, distinct, doall, repeatedly, expanding, fnil, keys, flatten, reverse, cat, concat, selectKeys, someFn, everyPred, cycle, range, butlast, expansive, groupBy, toObject, toArray} from './coll';
...
export const min          = curry(_.unspread(c.min), 2);
export const max          = curry(_.unspread(c.max), 2);
export const update       = subj(c.update);
export const updateIn     = subj(c.updateIn);
export const assocIn      = subj(c.assocIn);
export const getIn        = flip(c.getIn);
export const scan         = curry(c.scan);
export const best         = curry(c.best)
export const tag          = curry(el.tag, 2); //TODO first call is received as `partial`
